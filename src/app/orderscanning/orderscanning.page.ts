...
export class OrderscanningPage implements OnInit {
    ...
    if (ordervalue != "" && ordervalue != null) {
      ...
      this.ecoFlexService.ajaxCallService(orderscanning, "post", params).then(resp => {
        if (resp['status'].toLowerCase() != 'fail') {
          ...
          console.log(this.serialInputs.toArray());
          console.log(this.modelInputs.toArray());
          setTimeout(() => {
            // if (this.new) {
              ...
              for (let idx in this.itemLists) {
                if (items[0]['modelNumber'].toUpperCase() == this.itemLists[idx]['modelNumber'].toUpperCase()) {
                  if (!this.itemLists[idx]['isScanned']) {
                    ...
                    this.orderscanning.controls['container_' + index].setValue(items[0]['containerNumber']);
                    this.orderscanning.controls['binLoc_' + index].setValue(items[0]['binLocation']);
  ...
  handleItemScanner(type, index) {
    ...
    setTimeout(() => {
      ...
      if (type == 'cont') {
        ...
        this.orderscanning.controls['serialNo_' + index].clearValidators();
        this.orderscanning.controls['serialNo_' + index].setValue('');
        this.orderscanning.controls['serialNo_' + index].disable();
      }
      for (let idx of this.itemCount) {
        if (this.orderscanning.controls['modelNo_' + idx].value != '' && this.orderscanning.controls['modelNo_' + idx].value != null &&
  ...
  orderScanSubmit() {
      ...
      if (item['quantity'] == 1) {
        for (let idx of this.itemCount) {
          if (item['modelNumber'].toUpperCase() == this.orderscanning.controls['modelNo_' + idx].value.toUpperCase()) {
            ...
            item['containerNumber'] = this.orderscanning.controls['container_' + idx].value;
            item['binLocation'] = this.orderscanning.controls['binLoc_' + idx].value;
            item['isNewScreen'] = this.orderscanning.controls['serialNo_' + idx].value != '' && this.orderscanning.controls['serialNo_' + idx].value != null ? true : false
          }
        }
      } else {
        items.push(item);
      ...
      for (let i of this.itemCount) {
        if (item['modelNumber'].toUpperCase() == this.orderscanning.controls['modelNo_' + i].value.toUpperCase()) {
          ...
          itemLists.push({
            ...
            itemId: item['itemId'],
            quantity: 1,
            isNewScreen: this.orderscanning.controls['serialNo_' + i].value != '' && this.orderscanning.controls['serialNo_' + i].value != null ? true : false
          })
          console.log(this.orderscanning.controls['serialNo_' + i].value);
        }
      }
    })

    var save = this.ecoFlexService.baseUrl + this.ecoFlexService.saveorderScanold;
    let jsonobj = {
    ...
    this.ecoFlexService.present();
    this.ecoFlexService.ajaxCallService(save, "post", jsonobj).then(resp => {
  ...
  // Method to swith between serial & model entries
  switchSerialModel(evt) {
    let elementId = evt.currentTarget.id;
    let row = elementId.slice(-1);
    console.log(elementId);
    if (elementId.startsWith("serial")) {
      this.orderscanning.controls['modelNo_' + row].enable();
      this.orderscanning.controls['container_' + row].disable();
      this.orderscanning.controls['binLoc_' + row].disable();

    } else if (elementId.startsWith("model")) {
      this.orderscanning.controls['serialNo_' + row].enable();
      this.orderscanning.controls['container_' + row].enable();
      this.orderscanning.controls['binLoc_' + row].enable();

    }
  }

  //method to validate serial & model entry
  validateSerialModel(evt) {
    let value = evt.target.value;
    let elementId = evt.currentTarget.id;
    let row = elementId.slice(-1);
    console.log(value);
    ...
    if (value != '' && value != null) {
      if (elementId.startsWith('serial')) {
        this.orderscanning.controls['modelNo_' + row].disable();
        this.orderscanning.controls['container_' + row].disable();
        this.orderscanning.controls['binLoc_' + row].disable();

        this.orderscanning.controls['serialNo_' + row].enable();
        // this.modelEntry = false;
        // this.serialEntry = true;
      } else {
        this.orderscanning.controls['serialNo_' + row].disable();

        this.orderscanning.controls['modelNo_' + row].enable();
        this.orderscanning.controls['container_' + row].enable();
        this.orderscanning.controls['binLoc_' + row].enable();
        // this.serialEntry = false;
        // this.modelEntry = true;
      }
    } else {
      if (elementId.startsWith('serial')) {
        this.orderscanning.controls['serialNo_' + row].disable();
